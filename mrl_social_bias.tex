...
Figure~\ref{fig:rq3-correlation-dynamic} further visualizes this relationship by plotting the dynamic correlation between geometric concentration and variance concentration across prefixes. The two quantities are related but not interchangeable, as several prefixes deviate from a one-to-one coupling, consistent with $\alpha(d)\neq1$. This reinforces the main RQ3 insight that concentration in the bias direction can increase without a proportional increase in expressed variance. The \texttt{step7} filename is kept intentionally to preserve reproducibility traceability with the pipeline artifact naming.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{correlation_rq3_dynamic_step7.png}
\caption{Dynamic correlation view for RQ3 across prefix dimensions, comparing geometric concentration and variance expression behavior.}
\label{fig:rq3-correlation-dynamic}
\end{figure}
