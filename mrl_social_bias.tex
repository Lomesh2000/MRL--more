...
\begin{abstract}
Matryoshka Representation Learning (MRL) enables dynamic prefix truncation of embedding vectors, but its social-bias behavior across prefix dimensions remains underexplored. We study gender bias as the primary axis (with race as exploratory evidence) across MPNet-Standard, MPNet-MRL, Nomic-MRL, mxbai-MRL, Qwen3-0.6B-MRL, and Qwen3-4B-MRL. We first evaluate standard bias metrics across dimensions (WEAT, SEAT, StereoSet, CrowS-Pairs) and geometric concentration $C(d)$. We then introduce a variance-based concentration view $C_{var}(d)$ with bootstrap confidence intervals, and the anisotropy factor $\alpha(d)=C_{var}(d)/\|v_{1:d}\|^2$ to separate geometric bias mass from statistically expressed bias variance. The core finding is that early dimensions can appear bias-concentrated geometrically while simultaneously underexpressing bias variance ($\alpha(d)<1$), showing that concentration and expression are not equivalent. This offers a clearer measurement framework for fairness analysis in dimension-adaptive retrieval systems.
\end{abstract}
...
Let $f^{(d)}(x)$ denote the first $d$ dimensions of embedding $f(x)$, and let $v$ be a full-dimension bias direction estimated from attribute groups (e.g., male vs female).
...
Layer 2 & Cross-model generalization & \texttt{nomic/step8\_nomic\_results.json}, \texttt{mixbai\_mrl/step9\_mxbai\_results.json}, \texttt{qwen\_0.6\_emb.py/step10\_qwen3\_results.json}, \texttt{qwen-3b/step11\_qwen3\_4b\_results.json} \\
