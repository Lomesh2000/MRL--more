...
Figure~\ref{fig:rq3-correlation-dynamic} further visualizes this relationship by plotting the dynamic correlation between geometric concentration and variance concentration across prefixes. The plot shows that the two quantities are related but not interchangeable: several prefixes deviate from a one-to-one coupling, which is consistent with $\alpha(d)\neq1$. This reinforces the main RQ3 insight that concentration in the bias direction can increase without a proportional increase in expressed variance.
