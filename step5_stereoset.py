"""
Step 5 (Fixed): StereoSet Intrasentence Evaluation
Corrected dataset parsing for McGill-NLP/stereoset structure.
"""
...
print(f"Sentences dict keys: {list(sample['sentences'].keys())}")
print(f"Sentence list: {sample['sentences']['sentence']}")
print(f"Gold label: {sample['sentences']['gold_label']}")

# ============ STEREOTYPE SCORE COMPUTATION ============
def compute_stereoset_score(model, dim, max_examples=None):
    """
    Compute stereotype score at a given dimension.
    
    StereoSet structure:
    - context: sentence with BLANK
    - sentences['sentence']: list of 3 completion strings
    - gold_label: list of 3 ints [label_for_sent0, label_for_sent1, label_for_sent2]
    ...
    for example in examples:
        ...
        # Extract gold labels (nested in sentences dict)
        gold_labels = sentences_dict.get('gold_label', [])
        if len(gold_labels) != len(sentence_list):
            continue
        ...
        # Encode context and all sentences at target dimension
        ...
        # Find which completion is most similar to context
        best_idx = int(np.argmax(similarities))
        
        # Get the gold label for the chosen sentence
    ...
    stereotype_score = stereotype_count / total
    anti_score = anti_stereotype_count / total
    unrelated_score = unrelated_count / total
    
    return stereotype_score, anti_score, unrelated_score, total, stereotype_count
...
MAX_EXAMPLES = None  # None = full dataset; 500 = quick test

stereoset_results = {'std': [], 'mrl': []}

for dim in DIMS:
    print(f"Evaluating dimension {dim}... (this may take a moment)")
...
ax.axhline(y=1/3, color='black', linestyle=':', label='Random Baseline (1/3)', linewidth=1.5)
ax.set_xlabel("Dimension d")
ax.set_ylabel("Stereotype Score")
ax.set_title("StereoSet: Stereotype Score\n(Higher = More Stereotypical)")
ax.legend()
ax.grid(True, alpha=0.3)
ax.set_ylim(0, max(max(stereoset_results['std']), max(stereoset_results['mrl'])) * 1.2)

# Plot 2: Difference from Random
ax = axes[1]
...
plt.tight_layout()
plt.savefig("stereoset_fixed_step5.png", dpi=300, bbox_inches='tight')
print("\n✓ Plot saved to: stereoset_fixed_step5.png")
plt.show()

print("\n✓ Step 5 complete. StereoSet evaluation finished.")